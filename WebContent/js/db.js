...
DB.load = function () {
    ...
    alasql('CREATE TABLE stock(id INT IDENTITY, item INT, whouse INT, balance INT, obsoleteperiod INT, maxusage INT,' +
        ' leadtime INT, avgdailyusage INT, maxleadtime INT, reorderstatus INT, isobsolete INT);');
    ...
    alasql('CREATE TABLE supplier(id INT IDENTITY, whouseid INT, name STRING, email STRING);');
    ...
    alasql('DROP TABLE IF EXISTS bundletype;');
    alasql('CREATE TABLE bundletype(id INT IDENTITY, typename STRING, kindtext STRING, quantity INT, description STRING);');

    var pbundletype = alasql.promise('SELECT MATRIX * FROM CSV("data/BUNDLETYPE-BUNDLETYPE.csv", {headers: true})').then(
        function (bundletypes) {
            for (var i = 0; i < bundletypes.length; i++) {
                var bundletype = bundletypes[i];
                alasql('INSERT INTO bundletype VALUES(?,?,?,?,?);', bundletype);
            }
        });
